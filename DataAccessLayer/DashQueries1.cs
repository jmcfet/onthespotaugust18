    ...
    public class DashQueries
    {
        ...
        public List<qcsReportInfo> getAutoInfo()
        {
            ...
            filtered = (from auto in dbBCS.vAutosorts
                        where (auto.State > 0 && auto.State <  3 || auto.Slot == "     ")
                        && DbFunctions.TruncateTime(auto.DueDate) == day.Date
                        join qcs1 in dbBCS.QCSInfoes on auto.ArticleCode equals qcs1.HeatSeal into qcsjoin
                        from qcs in qcsjoin.DefaultIfEmpty()  //not in qcsinfo then missing
                        select new qcsReportInfo { storeid = auto.StoreID, CustomerID = auto.CustomerID,
                            InvoiceDate =auto.InvoiceDate,Description = auto.Description, rfid = auto.RFID,
           ...
           */
            //look in logger table for heatseal and if find 
            foreach (qcsReportInfo q in filtered)
            {

                if (q.qcsType == null)
                    q.qcsType = "Missing";
                var logs = (from log in dbBCS.vLogger
                            where (log.ArticleCode == q.HeatSeal)
                            && log.TimeStampIn > q.InvoiceDate
                            orderby log.MarkInDate descending
                            select log);

                //there might be multiple so take the one with latest markinDate
                v_Loggerall theone = logs.Take(1).SingleOrDefault();
                              
                q.TimeStampIn = null;
                if (theone != null)
                    q.TimeStampIn = theone.TimeStampIn;
